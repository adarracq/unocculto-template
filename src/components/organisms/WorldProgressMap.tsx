...
export default function WorldProgressMap({
    ...
    const { base, mastered, consolidated, urgent } = useMemo(() => {
        ...
        // 💡 CORRECTION 1 : Filtrer l'Antarctique (code AQ ou iso_a2_eh correspondant)
        // L'Antarctique cause 99% des problèmes de "terres étranges en bas" en projection Mercator.
    ...
    return (
        <View style={isBackground ? styles.backgroundContainer : styles.cardContainer}>
            <MapLibreGL.MapView
                ...
                scrollEnabled={true}
                zoomEnabled={true}
                // 💡 CORRECTION 2 : Restreindre le scroll (Bounding Box) pour éviter le wrapping
            ...
            >
                <MapLibreGL.Camera
                    ...
                    // 💡 Limiter la latitude pour empêcher d'aller voir le bas déformé
                    maxBounds={{
                        ne: [180, 85], // Nord-Est (Longitude, Latitude)
                        sw: [-180, -60] // Sud-Ouest (On bloque à -60 de latitude, juste au dessus de l'Antarctique)
                    }}
                />
                ...
                <MapLibreGL.ShapeSource id="baseSource" shape={base} onPress={onCountryPress ? handlePress : undefined}>
                    ...
                    <MapLibreGL.LineLayer id="baseLine" style={{ lineColor: THEME.colors.glass.background, lineWidth: 0.5 }} />
                ...
                {consolidated.features.length > 0 && (
                    <MapLibreGL.ShapeSource id="consolidatedSource" shape={consolidated}>
                        <MapLibreGL.FillLayer id="consolidatedFill" style={{ fillColor: THEME.colors.inProgress, fillOpacity: 0.4 }} />
                    </MapLibreGL.ShapeSource>
                )}
...
const styles = StyleSheet.create({
    backgroundContainer: { ...StyleSheet.absoluteFill }, // absoluteFillObject est plus sûr
